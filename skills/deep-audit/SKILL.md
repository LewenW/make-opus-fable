...
- Deterministic oracles are cheaper than any reviewer where they apply: run the type checker (`mypy --strict`), linter (`ruff`), and any property-based tests first, and treat their output as free findings. **But they cover a DIFFERENT bug class than model review, with near-zero overlap** (measured 2026-07-06): stock tools catch types, security patterns, dead code, unused variables, and naive-datetime *constructors* — they do NOT catch domain-contract violations (a `<` that should be `<=`, a missing `commit`, an off-by-one in a bucket key), because that code is syntactically valid and type-clean. On a 10-mutation set the full stock suite (ruff ALL + mypy strict + semgrep) caught 1/10 (an unused-unpacked-var tell that flagged a dropped bound) and recovered zero of the bugs model reviewers missed. So run oracles for their classes, but do not treat them as a substitute for the model reviewers on contract logic — the two are additive, not redundant. One genuinely useful free tell: an unused-unpacked-variable warning (ruff `RUF059`) reliably surfaces the "dropped upper bound / weakened guard leaves a variable unused" shape.
